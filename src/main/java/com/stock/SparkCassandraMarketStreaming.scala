...
case class MarketStreamData(bid_price: Double, order_quantity: Int, symbol: String, trade_type: String, timestamp: String)
case class MarketOrder(marketuuid: String, symbol: String, order_quantity: Int)
object SparkCassandraMarketStreaming {
  ...
  def main(args: Array[String]) {
    ...
    jsonf.print()
    val fields = jsonf.map(data => MarketStreamData(data("bid_price").toString.toDouble, data("order_quantity").toString.toInt, data("symbol").toString, data("trade_type").toString, data("timestamp").toString))
    fields.print()
    ...
    val results = fields.foreachRDD((recrdd) => {
      recrdd.toDF().registerTempTable("marketordertable");
      import com.gilt.timeuuid._
      val sqlreport = sqlContext.sql("select symbol, SUM(order_quantity) from marketordertable group by symbol order by symbol")
      //sqlreport.map(t => OutPut1(t(0).toString, t(1).toString.toInt)).collect().foreach(a => {})

      sqlreport.map(t => MarketOrder(TimeUuid().toString(), t(0).toString, t(1).toString.toInt)).saveToCassandra("marketspace", "market_order")
